...
public class GraficadorArbolAVL<T extends Comparable<T>> extends GraficadorArbolBinarioOrdenado<T> {
    ...
    private final int medidaContenidoVertice_BALANCE = 15;
    ...
    protected String graficaVertice(VerticeArbolBinario<T> vertice, int centroX,
        ...
        if (esDerecho(vertice))
            centroTextoX += (int) Math.ceil(balance.length() / 2) * medidaContenidoVertice_BALANCE;
        ...
        resultado += GraficadorSVG.graficaTexto(centroTextoX,
                        ...
                        medidaContenidoVertice_BALANCE, "black", balance);

        resultado += GraficadorSVG.generaCirculoConTexto(centroX, centroY, radio,
                "black", "white", medidaContenidoVertice, "black", vertice.get().toString());
